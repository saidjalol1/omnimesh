...
impl TransportUtils {
    ...
    pub fn create_runtime() -> Result<tokio::runtime::Runtime, String> {
        tokio::runtime::Runtime::new()
            .map_err(|e| errors::runtime_creation_failed(&e))
    }
    
    /// Safely acquires a mutex lock with standard error handling
    pub fn acquire_lock<'a, T>(mutex: &'a Mutex<T>) -> Result<std::sync::MutexGuard<'a, T>, String> {
        mutex
            .lock()
            .map_err(|e| errors::lock_acquire_failed(&e))
    }
}
...
pub mod logging {
    ...
    pub fn tcp_envelope_sent(addr: impl std::fmt::Display, active: usize, max: usize) {
        println!("TCP transport: envelope sent to {} (pool: {}/{})", addr, active, max);
    }
