...
fn persistent_dedup_cleanup_works() {
    ...
    for i in 0..10 {
        let msg_id = MessageId([i; 16]);
        assert!(
            dtn_store.has_seen_message(&msg_id),
            "Message {} should be marked as seen",
            i
        );
    }
...
fn ring_buffer_and_persistent_dedup_work_together() {
    ...
    for i in 0..100 {
        ...
        let mut payload_buf = PayloadStorage::<128>::new();
        payload_buf
            .push_bytes(format!("message {}", i).as_bytes())
            .unwrap();
        let envelope = SignedEnvelope::sign(header, payload_buf, &signing_key);
        ...
        assert_eq!(
            status,
            DeliveryStatus::Duplicate,
            "Message {} duplicate should be detected",
            i
        );
    }
    ...
    for i in 0..100 {
        ...
        assert_eq!(
            status,
            DeliveryStatus::Duplicate,
            "Message {} should still be detected as duplicate",
            i
        );
    }
