...
fn main() {
    ...
    let alice = OmnimeshClient::builder()
        .with_config(ClientConfig::development())
        .build()
        .unwrap();
    let bob = OmnimeshClient::builder()
        .with_config(ClientConfig::development())
        .build()
        .unwrap();
    let charlie = OmnimeshClient::builder()
        .with_config(ClientConfig::development())
        .build()
        .unwrap();
    let dave = OmnimeshClient::builder()
        .with_config(ClientConfig::development())
        .build()
        .unwrap();
    ...
    let run_agent = |name: &'static str,
                     client: OmnimeshClient,
                     target_did: omnimesh::envelope::Did,
                     is_starter: bool| {
        thread::spawn(move || {
            ...
            if is_starter {
                println!(
                    "🧠 [{} is kicking off the group chat as the Holder...]",
                    name
                );
                let prompt = format!(
                    "You are {}. Start a new conversation in a group chat with a short question (maximum 5 words) about a random topic. DO NOT write dialogue. DO NOT use your name. DO NOT use quotes.",
                    name
                );
            ...
            loop {
                if let Some(envelope) = client.receive_timeout(Duration::from_millis(100)) {
                    if let Some(PayloadKind::AgentCommand(cmd)) = envelope.payload.payload {
                        ...
                        let prompt = format!(
                            "You are {}. You are in a group chat. The last message was: '{}'. Reply with a very short statement (maximum 5 words). DO NOT write dialogue. DO NOT use your name. DO NOT use quotes.",
                            name, text
                        );
                        ...
                        let reply_msg =
                            payload::agent_command("chat", b"", display_text.as_bytes());
